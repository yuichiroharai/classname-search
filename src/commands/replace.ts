...
export async function replaceAction(
  ...
  console.log(
    `${commandVerb} ${matchCount} ${matchUnit} of "${classRegex}" with "${replacement}" in ${filteredFiles.length} ${fileUnit}.`,
  );
}
...
export const replaceCommand = addCommonArgs(new Command("replace"))
  ...
  .action(
    (targetGlob: string, classRegex: string, replacement: string, options) =>
      replaceAction(targetGlob, classRegex, replacement, options, "Replaced"),
  );
